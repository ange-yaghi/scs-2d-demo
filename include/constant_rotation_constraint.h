#ifndef ATG_SCS_2D_DEMO_CONSTANT_ROTATION_CONSTRAINT_H
#define ATG_SCS_2D_DEMO_CONSTANT_ROTATION_CONSTRAINT_H

#include "demo_object.h"

class ConstantRotationConstraint : public DemoObject {
    public:
        ConstantRotationConstraint();
        virtual ~ConstantRotationConstraint();

        virtual void initialize(atg_scs::RigidBodySystem *system);
        virtual void reset();
        virtual void render(DemoApplication *app);
        virtual void process(float dt, DemoApplication *app);

        atg_scs::ConstantRotationConstraint m_constraint;
};

#endif /* ATG_SCS_2D_DEMO_CONSTANT_ROTATION_CONSTRAINT_H */
